...
4. **启动 Agent Team 模式**：调用 Claude Agent Team 生成完整测试方案，使用不同的测试角色进行多角度分析
5. 输出到 `detail/program_<file_name>/` 目录

**Agent Team 测试角色配置**：
在生成测试方案时，启动以下测试角色进行协作分析：
- **功能测试专家**：负责分析核心功能、正常流程和业务逻辑
...
5. **启动 Agent Team 模式**：调用 Claude Agent Team 生成针对变更的测试方案，使用不同的测试角色进行多角度分析
6. 输出到 `detail/<file_name>/` 目录

**Agent Team 测试角色配置**：
在生成测试方案时，启动以下测试角色进行协作分析：
- **变更分析专家**：负责分析代码变更内容、影响范围和关联关系
...
| Agent Team 模式 | 多角色协作测试方案生成 | 利用不同测试专家的视角，提供更全面的测试覆盖 |
